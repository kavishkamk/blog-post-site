...
export const useAuth = () => {
    ...
    const [image, setImage] = useState("upload/images/unknownPerson.jpg");

    const logIn = useCallback((userId, token, image, expiration) => {
      console.log(image);
      setToken(token);
      setUserId(userId);
      setImage(image);
      ...
      localStorage.setItem("userData", JSON.stringify({userId, token, image, tokenExpirationDate: tokenExpirationDate.toISOString()}));
    } ,[]);
    ...
    useEffect(() => {
      ...
      if (storeData && storeData.token && new Date(storeData.tokenExpirationDate) > new Date()) {
        logIn(storeData.userId, storeData.token, storeData.image, new Date(storeData.tokenExpirationDate));
      }
    }, [logIn]);
    ...
    const logOut = useCallback(() => {
      setToken(null);
      setUserId(null);
      setImage("upload/images/unknownPerson.jpg");
